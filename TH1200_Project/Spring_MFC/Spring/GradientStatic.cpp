...
void CGradientStatic::OnPaint() 
{
	...
	if(m_iAlign == 1) // center
		::DrawText(dc, m_sTEXT, -1, &rect, DT_VCENTER|DT_CENTER|DT_WORDBREAK);
	else if(m_iAlign == 0) // left
	{
		rect.left+=m_iLeftSpacing;
		::DrawText(dc, m_sTEXT, -1, &rect, DT_VCENTER|DT_LEFT|DT_WORDBREAK);
	}
	else //right
	{
		rect.right-=m_iLeftSpacing;
		::DrawText(dc, m_sTEXT, -1, &rect, DT_VCENTER|DT_RIGHT|DT_WORDBREAK);
	}
...
void CGradientStatic::SetWindowText(LPCWSTR a_lpstr)
{
